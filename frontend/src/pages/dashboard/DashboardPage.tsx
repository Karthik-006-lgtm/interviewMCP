...
export function DashboardPage() {
  ...
  if (loading) {
    return <div className="glass-panel rounded-[1.75rem] p-6 text-gray-700">Loading dashboard insights...</div>;
  }
            ...
            <div className="space-y-5">
              <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
                <div className="app-surface rounded-[1.5rem] p-5">
                  <p className="font-display text-lg text-gray-900">{resume.candidateName || "Latest candidate profile"}</p>
                  <p className="app-muted mt-2 text-sm">{resume.originalFileName}</p>
                  <p className="mt-5 text-4xl font-bold text-teal-700">{Math.round(resume.readinessScore)}%</p>
                  <p className="app-muted mt-3 text-sm">Readiness score</p>
                </div>
                <div className="app-surface rounded-[1.5rem] p-5">
                  <p className="font-display text-lg text-gray-900">Profile summary</p>
              ...
              <div className="app-surface rounded-[1.5rem] p-5 text-sm text-gray-800">
        ...
        <SectionCard title="Priority Roadmap" subtitle="What to improve next based on your latest analysis.">
          {recommendations ? (
            <div className="space-y-4">
              <div className="app-surface rounded-[1.5rem] p-5">
                <p className="font-display text-lg text-gray-900">Improvement roadmap</p>
              ...
              <div className="app-surface rounded-[1.5rem] p-5">
                <p className="font-display text-lg text-gray-900">Learning suggestions</p>
                <ul className="app-muted mt-4 space-y-2 text-sm">
              ...
              <div className="app-surface rounded-[1.5rem] p-5">
                <p className="font-display text-lg text-gray-900">Why this matters</p>
          ...
          <div className="space-y-4">
            {metrics?.recentReports.length ? (
              metrics.recentReports.map((report) => (
                <div key={report.id} className="app-surface rounded-[1.5rem] p-5 text-gray-700">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="font-display text-lg text-gray-900">{report.title}</p>
                  ...
                  <p className="mt-4 text-sm text-gray-700">{report.executiveSummary}</p>
                  <p className="mt-3 text-sm text-gray-700">{report.progressSummary}</p>
                </div>
              ))
            ) : (
              <div className="app-surface rounded-[1.5rem] p-5 text-sm text-gray-700">
